...
- Runtime tests on 2026-07-22 showed that adding `Field.options.dataFillList` to a generated DBA is not by itself proof that data fill will execute. The package imported and the selector could show real product/material options, but target fields stayed blank in the add form. Existing native samples such as BOM from a platform-authored app can work, which implies the designer save/import path may register additional trigger metadata or require exact platform-owned association structures. Treat generated `dataFillList` as `can_generate` only until a browser test selects a seeded record and shows target fields changing in the active form, not merely in the background list.
- Runtime designer tests on 2026-07-22 showed that ordinary runtime forms can open while the form designer canvas renders only the last top-level component. The observed generated package used `styleDetail[].col` as a 12-grid offset (`0`, `6`) while platform exports use `col` as the ordinal position within the row (`0`, `1`, `2`) and `size` as the width. Generate `styleDetail` with row ordinal grouping: two half-width fields should be `(row=n,col=0,size=6)` and `(row=n,col=1,size=6)`, three third-width fields should use `col=0/1/2,size=4`. Do not use Bootstrap-style offsets such as `col=6`. Browser-test the form designer after import; runtime list success is not proof that designer layout is editable.
- Runtime tests on 2026-07-22 showed that child-table metadata can be structurally valid while the end-user add form still has no ordinary editable row. Adding `defaultAddRow`, `defaultAddRowNum`, child table/field `options.addOption`, form-level `childAuthAdd`, and related child permission flags did not make the BOM detail table show a normal single-row add affordance; only `粘贴新增` was visible. Treat those flags as metadata hints only. A child-table requirement is not `runtime_verified` until a real add/edit form shows an editable detail row or ordinary row-add control, the tester can select/fill a child row, submit it, and query child records by parent `pid`.
- Runtime tests on 2026-07-22 showed that `styleDetail` is not only designer layout; runtime form controls can use its mirrored component options. A BOM package where `fields[]` had `启用/停用` and `digitalformat.options.point = 3` still rendered `是/否` and saved child quantity `0.035` as `0` because the matching `styleDetail` entries still had the old select options and `point: 0`. For every field option or precision change, patch parsed `styleDetail` entries with the same `model` as well as `fields[]`, including nested child-table `columns[]` entries, then import and verify through create plus edit-page回查.
...
| Form design | Text/number/date/select/radio/checkbox/address/attachment/image/rich text/location/signature/OCR/system fields/related records/child tables/summary/amount uppercase/geofence/embed/AI analysis | Clone component metadata from proven samples and keep DDL aligned | Add/edit one record per important component and inspect stored payload. For child tables, first prove the runtime form exposes an editable row or ordinary row-add control; `粘贴新增` alone is not enough. |
...
| Related data | Related record, related multi-select, data fill, data linkage, related lists | Use `aboutTable`, helper ref columns, selector `dataFillList`, related-list settings, and any extra trigger/association metadata discovered from a platform-saved sample | Select seeded data in UI, verify auto-filled fields and `_ref_id` payload. Static `dataFillList` in DBA JSON is not enough. |
| Related selector filters | Fixed-value filtering of related-record choices by target fields such as product/material type | Form-level `associationOptions.filterGroups` cloned from platform exports. For fixed select filters, use `valueType: "CUSTOMIZE"` and array values matching target `mamselect` storage. For child-table selectors, keep the child table `tableName` on the association option. | Runtime-open selector with seeded records of both allowed and disallowed types; allowed rows must appear and disallowed rows must not. Observed 2026-07-22: imported BOM parent selector showed only `成品/半成品`; BOM child selector showed only `原料/辅料/包材`. |
...
| Data fill / auto carry-over | `options.dataFillList` cloned from proven selector structure, plus any additional trigger/association metadata proven by a platform-saved sample. | Select a customer/product/supplier/order and verify filled fields in form and submitted payload. Do not claim completion from static `dataFillList` alone. |
| Child table storage | `childrenTable`, child DDL, `pid`, child field columns, print/query relation by parent id. | Runtime add/edit form must expose an editable row or ordinary single-row add control, then save a parent with multiple child rows and query child rows by `pid`. `defaultAddRow`/`childAuthAdd`/`options.addOption` metadata and `粘贴新增` visibility alone are only `can_generate`. |
...
- Do not claim runtime success from `dataFillList` metadata alone. Import and create seed master data, select a related record in the UI, then verify the filled values appear in the active add/edit form and in the submitted payload.
- When testing carry-over in a drawer over a list page, distinguish active-form values from background-list values. A page-wide text search can find the selected product code/specification because an existing list row is visible underneath; that is not proof of data fill. Read the active drawer's input values or submit payload.
