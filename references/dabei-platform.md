...
- Runtime tests on 2026-07-22 showed that adding `Field.options.dataFillList` to a generated DBA is not by itself proof that data fill will execute. The package imported and the selector could choose a real product, but target fields stayed blank. Existing native samples such as BOM still work, which implies the designer save/import path may register additional trigger metadata or require exact platform-owned association structures. Treat generated `dataFillList` as `can_generate` only until a browser test selects a seeded record and shows target fields changing.
