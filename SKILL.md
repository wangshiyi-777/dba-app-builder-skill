...
9. When a required native designer feature lacks a proven, exportable DBA sample, immediately ask the user to save one minimal platform-side example. Give the exact form, designer page, and values to set. After the user confirms, export that app, diff the package and capture the save/query API shape, then promote only the verified structure into reusable generation rules. Do not wait on an ambiguous "open the page" step or substitute a fake field/module.
...
- In `styleDetail`, every `childrenTable.listName` must resolve to that same node's `model`, and each resolved child-table `listName` must be unique across the app. The platform exporter strips table-name prefixes before building its symbol map; reusing a list name can make an imported app run normally but fail export with `Duplicate key SymbolEntry(type=TABLE_NAME, ...)`. Treat this as a blocking package-validation error, not a cosmetic layout difference.
- Child-table editability is a runtime behavior, not a DDL/layout fact. `defaultAddRow`, `defaultAddRowNum`, child-field/table `options.addOption`, and form-level `childAuthAdd` may import without producing an editable detail row. Do not call a child-table feature complete until runtime shows an editable row or ordinary single-row add control; `粘贴新增` is only a bulk-paste entry and is not equivalent.
- Runtime form rendering can read field component options from `styleDetail`, including nested child-table column definitions, rather than only from `fields[]`. When changing select options, number precision such as `digitalformat.options.point`, requiredness, default values, widths, or child-table column behavior, update both the canonical `fields[]` object and every matching `styleDetail` mirror. In a 2026-07-22 regression, BOM `fields[]` had `启用/停用` and `point: 3`, but runtime still showed `是/否` and saved `0.035` as `0` because `styleDetail` still had the old options.
- Newly added physical fields must be real design fields, not only layout controls. Use `FIELD` symbol entries for every field `id/fieldKey`, set non-virtual business fields `referenced: true`, add the field to `fields[]`, DDL, `styleDetail`, and form-level `addOption/editOption`. A 2026-07-22 regression showed that fields added with `COMMON` symbols or `referenced: false` were visible in the runtime drawer through `styleDetail`, and related-record data fill appeared in the input boxes, but `/api/design/forms/{form}/fields` did not register them and submit/API update ignored their values. Treat `/api/design/forms/{form}/fields` presence plus `getById` saved-value回查 as the proof that a new field is truly writable.
...
- For related-record selector fixed filters on Dabei/K6, prefer a platform-exported `associationOptions.filterGroups` shape when available. A 2026-07-22 runtime test proved that form-level `associationOptions` entries can filter imported `aboutTable` selectors by target fixed values: `BOM清单.父产品` showed only `成品/半成品`, and child-table `BOM明细.子件物料` showed only `原料/辅料/包材`. Use `valueType: "CUSTOMIZE"` and array values for `mamselect` targets, keep `showFieldId`, `mainFieldId`, `foreignFormId`, and child `tableName` aligned, and still runtime-test the actual selector candidates after import.
- For child tables, verify the runtime add/edit form has an ordinary editable detail row or single-row add affordance before testing line-level selectors. If the table only exposes `粘贴新增`, mark the line-entry UX as not runtime verified even when `childrenTable`, child DDL, `defaultAddRow`, `options.addOption`, or `childAuthAdd` exist.
...
- When a dropdown or numeric precision looks correct in local `fields[]` but wrong in the browser, inspect parsed `styleDetail` mirrors before assuming import cache or frontend bugs. Retest with a real record and edit-page回查; create-page display alone can miss saved-value truncation.
